...
export { isBareMutationDecline, isExplicitMutationApproval } from './approval-policy'
import { evaluateMutationApproval, failsMutationApprovalGuards } from './approval-policy'
...
export async function authorizesPendingMutation(input: string): Promise<boolean> {
  return evaluateMutationApproval(input, isSemanticMutationApproval)
}
