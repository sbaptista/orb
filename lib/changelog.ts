...
export const CHANGELOG: Release[] = [
  {
    ...
    date: '2026-09-25',
    changes: [
      'Expanded model choice by role. Operational, strategic, and evaluation settings now offer every catalog model that supports the role, Kimi K3 no longer carries a separate experimental badge, and live voice offers both applicable OpenAI Realtime models with `gpt-realtime-2.1-mini` as the default. Gemini operational turns now preserve signed reasoning across tool continuations.',
      'Removed hidden provider pins from paid conversation diagnostics. An approved eval case now uses the Evaluation Model selected in Settings unless the operator supplies an explicit provider/model override; model identity and catalog coverage moved into the zero-cost interaction verifier.',
      'Added correlated latency traces for every admitted text and voice turn. One durable turn UUID now joins Realtime speech detection, transcription, shared browser submission, server authentication/context/routing/model/tool/confirmation/persistence stages, committed refreshes, response paint acknowledgement, first audio, playback completion, and microphone return.',
      'Conversation diagnostic exports now include the correlated timing records. The client records these traces even when optional performance sampling is disabled, while production accepts the always-on diagnostic rows only from an authenticated Orb conversation.',
      'Corrected voice timing to end when audio playback actually finishes. It previously labeled model generation completion as microphone return even while Orb was still speaking, which understated voice latency and made the trace misleading.',
      'Fixed interrupted responses attempting to acknowledge an event that was never stored. The server now exposes a response event ID only after the matching assistant event is durably persisted; recoverable receipt-presentation failures remain unacknowledged until snapshot repair creates the event.',
      'Used the first correlated trace to remove avoidable conversation latency. Confirmations now execute the stored proposal before loading model policy, the full context packet, routing, budgets, or a model; tool continuations no longer wait through an artificial 600 ms pause; and mutation preparation and persistence have separate timing stages.',
      'Correlated conversation diagnostics now batch through the existing telemetry queue instead of issuing separate immediate database writes for server timing, browser completion, acknowledgement, and microphone return while speech is starting or playing.',
      'Fixed “OK, stop” becoming a new model turn and producing an unrelated answer. The shared text/voice stop grammar now accepts ordinary conversational prefixes, while preserving “no” as an answer when nothing is running.',
      'Fixed a merged voice transcript reusing the interrupted fragment’s durable turn ID. Fragment and replacement now have distinct turns, so the merge interrupt hides only the obsolete fragment instead of removing both messages from restored history.',
      'Reduced development startup contention without weakening cache recovery. Orb keeps the established clean `.next` start that prevents stale compiled CSS, while system-state checks no longer create database telemetry, overlap one another, or rerender the full provider tree solely to record an unused poll timestamp or an unchanged broadcast.',
      'Replaced the 20-query eager conversation packet with a compact working context: accessible active project names/codes/owners and the current project’s open/in-progress todos. Parked, closed, dormant, cross-project, knowledge, audit, ticket, user, invitation, category, group, role, platform, and friction data now load only through the relevant read tool. Complete status tables still perform an authoritative lazy read, and the unsafe voice summary shortcut no longer treats partial data as complete.',
      'Made interruptions explicit across the compact-context coordinator. Orb now checks the durable turn after context assembly, before and after every provider round, around lazy read tools, and before proposal persistence, so a stopped turn cannot continue into another model call or a new confirmation proposal. Each checkpoint reads conversation status and matching interrupt events in one database request; an already committed mutation receipt remains durable.',
      'Made development restart recovery survive React remounts. The browser now remembers the last dev-server boot in session storage and performs one automatic reload when a rebuilt server becomes reachable, preventing an old RSC tree from repeatedly falling back to failed navigation after `.next` is cleaned.',
      'Fixed duplicate transcript identities during voice replay. The voice boundary admits each correlated provider turn only once per mounted session, and final response reconciliation collapses local placeholders onto one durable event id before React renders them.',
      'Fixed direct list and count requests disappearing behind aggregate-grounding repair. Once any structured read tool returns a trusted packet for an explicit list, show, display, see, or count request, Orb now renders the records and authoritative total immediately instead of paying for another model synthesis that can distort or reject them. Analytical requests still continue through the model.',
      'Expanded voice restart replacement to plain “try again …” phrasing. A corrected request now replaces the unfinished transcript instead of being appended to it repeatedly.',
      'Corrected direct todo-list scope and ordering. An unqualified read now defaults to the selected project, explicit cross-project ownership remains available, and query_todos exposes deterministic created/updated/closed/due/number/priority sorting so “most recent,” “newest,” LIFO, FIFO, and oldest requests are enforceable rather than left to database row order.',
      'Narrowed the deterministic status-breakdown route. A request for closed todo records in a table now reaches the record query; only count/breakdown language or a table spanning multiple statuses produces the aggregate status table.',
      'Model-free interaction verification, TypeScript, focused ESLint, and diff checks passed once after the latency repairs. Focused ESLint reported zero errors and six pre-existing dashboard warnings. No paid model eval was run; live timing acceptance remains required.',
    ],
  },
  {
    version: 'v0.6.336',
  ...
  },
  {
    ...
    date: '2026-09-06',
    changes: [
      'Corrected a claim made in the previous release and repeated in four places: that placing the read-only data broker under system ownership means a development AI can no longer rewrite it. Stan asked the obvious follow-up — how does the broker get maintained, then — and the answer disproves the claim. The working copy in the repository is writable by any AI tool and is edited routinely; installing a new version is a privileged command that an AI composes and Stan runs, so without reading the difference first the AI\'s version becomes the one that runs; and an AI can skip the installed copy entirely by running the working copy directly, which was demonstrated against the live database.',
      'What system ownership actually buys is that an unannounced change to the installed copy is detected, not prevented. That is worth having and is much smaller than what was claimed. Recorded as a standing finding so the same overstatement is not made again.',
      'Worth noting alongside it: rewriting the broker would not grant write access to anything, because read-only is enforced by database permissions rather than by the broker. It would only bypass the broker\'s own filtering of deleted rows and its activity log — both of which an existing accepted finding already records as reachable by connecting to the database directly. Internal developer tooling and documentation: no application route, component, or user-facing behavior changed. Eval: not applicable — no Orb-conversation capability, tool, routing rule, prompt, or defined speech behavior changed.',
    ],
  },
  {
    version: 'v0.6.309',
  ...
  },
  {
    ...
    changes: [
      ...
      'Added the read-only agent broker to the set of installed programs whose integrity is checked, now that it is installed under system ownership, so an unannounced change to the installed copy is detected on every run. (This entry originally claimed something stronger and untrue — that a development AI can no longer rewrite its own broker. Corrected the same day in v0.6.310.) Internal developer tooling and documentation: no application route, component, or user-facing behavior changed. Eval: not applicable — no Orb-conversation capability, tool, routing rule, prompt, or defined speech behavior changed.',
    ],
  },
  {
    version: 'v0.6.307',
    date: '2026-09-06',
    changes: [
      'Made every exposed conversational create, update, or delete require a distinct confirmation turn in both text and Realtime voice: todo and project CRUD, Knowledge Repository create/update, and ticket creation, plus todo moves and closes. Permission bundled into the requesting turn no longer crosses the commit boundary, including saved allow/session preferences; the first turn only stores the exact proposal, and a later explicit approval executes it. Ticket creation now has its own replay-safe database confirmation routine, with notifications dispatched only after a new receipt; deterministic internal incident filing remains direct and outside this user-command contract. A latest-request rule prevents an unsupported completion claim in older history from distracting the model from a new mutation request. Capability scope is otherwise unchanged: Knowledge still has no delete, audit remains read-only, and tickets still have no conversational update/delete.',
      'Added dedicated admin-only read tools for registered users and invitations in both text and Realtime voice. Both channel adapters call the same bounded server functions, expose only reviewed directory fields, keep both tables out of generic model-authored SQL, and preserve the existing rule that non-super-admin administrators cannot read super-admin user rows. Fixed the Realtime bridge dropping those structured rows while claiming their tables were already on screen: todo, user, and invitation packets now pass through one shared packet-to-Markdown formatter into the same OrbConversation renderer text already uses. The Voice prompt may only claim a table is visible after a supported list tool returned rows in that turn.',
      'Made query_repository.source required in the canonical generated tool schema. A full Tier 1 run selected the correct repository tool for an explicitly local inspection but omitted the optional source field; runtime defaulting would still have read local source, yet the tool contract and existing regression case require preserving the user\'s explicit environment choice. The v0.6.306 baseline had the same optional schema and assertion, so the failure was pre-existing rather than introduced by this Voice change.',
      'Aligned two mutation-safety eval fixtures with the server-held confirmation contract. The legacy mutation-approval case expected the model to ask for confirmation before calling a tool, directly contradicting the current tool-first rule; it now makes a one-run exact-delete tool assertion and leaves confirmation to the server. The ambiguous-title behavior case now supplies two exact duplicate visible titles instead of three loosely similar ones, removing a reasonable search-first interpretation. Stan\'s minimal rerun passed: create-after-hallucinated-history 2/3, ambiguous-todo-title-does-not-mutate-silently 3/3, and mutation-approval 1/1.',
      'Audited the database paths rather than treating matching tool names as proof. Todo/project/knowledge mutations already converge on the shared proposal store and confirmation dispatcher; conversational ticket creation and the new user/invitation reads now also converge across text and Realtime. Existing read adapters for todos, projects, Knowledge, tickets, and audit remain channel-specific, so the broader claim that all text and voice database access uses one path is not true. Updated the generated contract, Realtime schemas/client adapter, prompt policy, capability matrix, and matching eval cases. Deterministic checks passed once. Stan applied the ticket-confirmation migration and its closing query confirmed the intended proposal constraint and service-only execution boundary. Authenticated text and Realtime acceptance passed for users/invitations reads, separate-turn Knowledge C/U, confirmed ticket C, rejected ticket C, and the repaired Realtime table display. Read-only broker checks independently confirmed the Voice Knowledge update and `TICKETS-74`, with no rejected suggestion ticket. Model gates passed at their required thresholds: the post-extension Tier 1 run passed every case except the pre-existing optional query_repository source ambiguity, whose required-schema focused rerun passed 1/1; mutation-safety Tier 2 passed after the minimal targeted rerun.',
    ],
  },
  {
    version: 'v0.6.305',
