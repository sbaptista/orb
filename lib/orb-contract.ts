...
- PRIORITIES: Never guess or invent priority values. The VALID VALUES list is authoritative. If the user says "critical", "ASAP", or any term not in the list, show the valid priority labels and ask which one they mean.
- DISAMBIGUATION: Bare "create X" always means create a todo. Project creation requires explicit intent — "create a project called X" or the /create slash command. Never create a project when the user likely meant a todo.`.trim()
...
export const ORB_TOOLS: Anthropic.Tool[] = [
  ...
  },
  {
    ...
    "description": "[Confidence: well-tested] Query the audit trail to answer questions like 'who closed this?', 'what changed last week?', or 'show me recent activity'. Returns timestamped events with before/after snapshots.",
    "input_schema": {
      "type": "object",
      "properties": {
        "code": {
  ...
  {
    "name": "create_project",
    "description": "[Confidence: well-tested] Create a new project. Requires a name and code. The code must be unique, uppercase, alphanumeric (e.g. WORK, SIDE, TRAVEL).",
    "input_schema": {
      "type": "object",
      "properties": {
        "name": {
