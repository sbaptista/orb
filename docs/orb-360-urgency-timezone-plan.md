...
**Eval-case deviation — RESOLVED in ORB-361 Phase 3.3 (2026-07-28).** The promised Tier 2 case was not writable at the time: the eval harness's `backlogOverride` blanked the server-computed project-health packet (`app/api/orb-eval/route.ts`), so a due-dated todo could not be seeded into the surface this change affects. Phase 0 changed no tool, param, routing rule, or defined speech policy (data plumbing only), which was within the suite's stated boundary for "no case required", and the behavioral case was moved to ORB-361 Phase 2/3 where the harness would need a seeding mechanism anyway. **That mechanism now exists** — `projectHealthOverride` seeds the packet directly — and Phase 3.3 adds the two cases (`orb-mood-names-the-driving-task`, `orb-mood-calm-project-has-no-invented-cause`).
