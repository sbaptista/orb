...
| [`app/api/orb-realtime/turn/route.ts`](../app/api/orb-realtime/turn/route.ts) | Realtime adapter: authenticates tool calls, reads live facts, resolves channel-specific tool inputs, and delegates proposal persistence/capabilities/confirmation to `lib/orb-operations/`. |
| [`lib/orb-operations/`](../lib/orb-operations/) | ORB-342 canonical operation spine shared by serial text and Realtime voice: signed capabilities, proposal persistence, confirmation, and serial todo normalization. |
...
| [`scripts/migrations/20260729_orb_342_canonical_proposals.sql`](../scripts/migrations/20260729_orb_342_canonical_proposals.sql) | Makes the proposal spine transport-neutral with serial/Realtime channel provenance, shared pending summaries, richer serial todo fields, and the shared dispatcher. |
...
**Batch todo mutations (updated by ORB-342, 2026-07-29):** both engines now persist the same `batch_todo_action` proposal and confirm it through `confirm_realtime_batch_todo_mutation`. The transaction is all-or-nothing, stale-checks every target, writes one audit row per operation, stores one replay-safe receipt, and preserves the richer serial description/priority/due/reminder fields. Closing remains singular because each close requires resolution notes and a knowledge entry.
