...
This is an umbrella interaction-quality workstream, not a single feature. It is tracked as **ORB-317 — Strategic Orb v1 interaction-quality program**. It absorbs and coordinates slices such as context/eval architecture, operating-rule cleanup, capability-matrix gaps, project-health reads, next-step recommendations, and future interaction improvements. Individual ORB todos can still track narrow implementation pieces, but the product direction should be understood as one program: make Orb a reliable strategic interaction layer rather than a generic task chatbot.
...
**Implementation note, v0.6.159:** Next-Step Read is now the second enabled Strategic Orb v1 behavior. Prompts such as "what should I work on next?", "where should I focus?", and "help me prioritize" have a semantic contract that keeps recommendations compact, current-user-owned, evidence-labeled, and free of invented blocker/dependency claims. A v0 Next-Step Packet now derives a bounded candidate set from existing project-health, active task, priority, due-date, stale-work, and recent-audit data. The packet is advisory: scores and signals shape the recommendation surface, but Orb must still label judgment and avoid treating packet cues as commands.
