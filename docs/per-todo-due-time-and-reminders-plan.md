...
**~~Known limitation, accepted for v1~~ — retired at build time (2026-07-26).** Stan hit the gap immediately ("I can't even select Boston"), so Phase 1 ships a **vendored city database** instead: `scripts/generate-city-zones.mjs` pulls the GeoNames cities15000 dump (public domain, CC-BY attribution kept in the file), filters to population ≥ 15k — the floor chosen because 50k's first casualties were Hawaiian towns (Kailua ~40k) — and commits `lib/data/city-zones.json` (33k+ cities, ~400 KB gzipped). `lib/city-zones.ts` dynamic-imports it on first picker focus, with the bare IANA list as the never-dead fallback while it loads; population ordering puts Boston MA above Boston UK without a ranking pass, and US/CA/AU entries carry state/province disambiguation ("Vancouver, BC" vs "Vancouver, WA").
