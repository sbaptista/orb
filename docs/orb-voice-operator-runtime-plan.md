...
v0.6.119 followed up with a systematic audit (not just the one instance) of `app/actions/orb-converse.ts` against its eval mirror `app/api/orb-eval/route.ts`, looking for other logic the two files were supposed to keep identical but had drifted or gapped independently. Found and fixed: the eval copy's `VOICE CONVERSATION` prompt block was missing production's "never re-greet," brevity-threshold, and no-filler-phrases rules entirely — meaning a regression in any of those speech-policy behaviors was structurally invisible to the eval suite (no Tier 2 case could ever fail against a rule the eval prompt didn't contain). Extracted into a shared `buildVoiceConversationPrompt` in `lib/orb-prompt.ts`. Also found: `propose_adaptation` and its supporting prompt text (`ORB_COMMITMENT_INTEGRITY`, `ORB_ADAPTATION_BEHAVIOR`) were entirely absent from the eval route — not drifted, never wired in — so that whole capability was untestable; now wired in with a first eval case. Remaining, larger, and deliberately deferred: the eval route's context/backlog-building (~450 lines) is still an independent hand-written parallel of production's `buildContext`, not a shared call — the real structural fix would consolidate that too, but it's a bigger refactor than this pass and wasn't attempted here.
