...
**Accepted regression:** a breach is now reported up to ~24h later instead of ~15 minutes. Since spend only moves during working sessions, in practice that means learning the next morning rather than mid-session.

**Follow-up for Stan (not done by the AI):** the `CRON_SECRET` GitHub repository secret is now unused and should be deleted from repo settings. The Vercel production env var must stay — Vercel Cron sends it.
